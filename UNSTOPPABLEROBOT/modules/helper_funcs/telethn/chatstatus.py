from UnstoppableRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from UnstoppableRobot import DRAGONS
...
async def Unstoppable_is_admin(chat_id: int):
    status = False
    Unstoppable = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Unstoppable.id == user.id:
            status = True
            break
    return status
